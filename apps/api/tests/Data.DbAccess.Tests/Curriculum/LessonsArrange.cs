...
internal static class LessonsArrange
{
    ...
    public static async Task<LessonId> InsertLessonAsync(
        IDbContextFactory<MileseDbContext> dbContextFactory,
        ConceptId conceptId,
        string title,
        CancellationToken cancellationToken = default)
    {
        ...
        var lesson = new LessonDb
        {
            ConceptId = conceptId,
            Title = new LessonTitle { Value = title },
            EstimatedMinutes = new EstimatedMinutes { Value = 10 },
            Order = new SortOrder { Value = 1 },
        };
